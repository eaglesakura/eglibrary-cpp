...
public:
    ...
    void updateViewport(const FitType_e type) {
        ...
        if (type == FitType_Auto) {
            scaling = ::std::min<float>(mulX, mulY);
        ...
        //! ジャストフィット用の補正を行う。
        if (es::equals(scaling, mulX) && !es::equals(drawingArea.width(), realDisplaySize.x)) {
            drawingArea.right = realDisplaySize.x;
        }
        if (es::equals(scaling, mulY) && !es::equals(drawingArea.height(), realDisplaySize.y)) {
            drawingArea.bottom = realDisplaySize.y;
        }

        //! 描画位置をセンタリングする
        const int offsetX = (int)(realDisplaySize.x - drawingArea.width());
        const int offsetY = (int)(realDisplaySize.y - drawingArea.height());
     ...
     * @return 描画エリアの幅
     */
    int getDisplayViewportWidth() const {
     ...
     * @return 描画エリアの高さ
     */
    int getDisplayViewportHeight() const {
     ...
     */
    int getVirtualDisplayCenterX() const {
     ...
     * @return 仮想ディスプレイのX中央値
     */
    int getVirtualDisplayCenterY() const {
        return (int) virtualDisplaySize.y / 2;
    }

    int getRealDisplayWidth() const {
        return (int) realDisplaySize.x;
    }

    int getRealDisplayHeight() const {
        return (int) realDisplaySize.y;
    }
     ...
     * @return 画面外だったらtrue
     */
    bool isOutsideReal(const Vector2f &realPos) const {
     ...
     * @return 外側だったらtrue
     */
    bool isOutsideReal(const int left, const int top, const int width, const int height) const {
     ...
     * @return
     */
    bool isOutsideVirtual(const int left, const int top, const int width, const int height) const {
