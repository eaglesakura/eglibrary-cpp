 ...
 */
inline JavaVM *getThreadJavaVM() {
    return ::jc::__private_jni::JniSupportArgs<void>::sJavaVM;
}
...
// リファレンスを取得する
     ...
     */
    JavaObjectWrapper &multiThread(const bool set) {
        setMultiThreadAccess(set);
        return *this;
    }
...
class JavaStringWrapper : public JavaObjectWrapper {
    void initialize(const char *str, JNIEnv *env) {
        if (!env) {
            env = ::jc::jni::getThreadJniEnv();
        }
...
public:
    ...
    JavaStringWrapper(const ::std::string &str, JNIEnv *env = nullptr) :
            JavaObjectWrapper() {
        initialize((const char *) str.c_str(), env);
    }
