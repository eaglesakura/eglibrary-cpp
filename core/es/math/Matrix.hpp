 ...
 * row     0 0 0 1
 */
template<uint ROW, uint COLM, typename T = float>
struct Matrix {
    T m[ROW][COLM];
     ...
     * 単位行列であることを確認する
     */
    inline bool isIdentity(const float threshold = 0.001f) const {
        for (int i = 0; i < ROW; ++i) {
            for (int k = 0; k < COLM; ++k) {
                if (i == k) {
...
//        const float f = (float) (1.0 / (tan(degree2radian(fovY_degree)) / 2.0f)); // 1/tan(x) == cot(x)
    ...
    inline void copy(const Matrix<4, 4> &origin) {
        for (int r = 0; r < ROW; ++r) {
            for (int c = 0; c < std::min<uint>(COLM, 4); ++c) {
                m[r][c] = origin.m[r][c];
            }
        }
    }
    ...
    inline void copy(const Matrix<4, 3> &origin) {
        for (int r = 0; r < ROW; ++r) {
            for (int c = 0; c < std::min<uint>(COLM, 3); ++c) {
                m[r][c] = origin.m[r][c];
            }
        }
    }
